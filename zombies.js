...
function Player(name, health, strength, speed) {
  ...
  Player.prototype.takeItem = function(item) {
    if (this._pack.length >= 3) {
      ...
      console.log(
        Player.name + ", " + item.name + " has successfully been stored."
      );
  ...
  Player.prototype.discardItem = function(item) {
    ...
    if (index >= 0) {
      ...
      console.log(Player.name + ", " + item.name + "was discarded.");
      return true;
    } else {
      console.log(
        item.name +
          " was unable to be discarded, because the item could not be found."
      );
      return (index = -1);
    }
  };
  ...
  Player.prototype.equip = function(itemToEquip) {
    ...
    if (index === -1) {
      ...
      let itemHeld = this.equipped;
      this.equipped = itemToEquip;
      this.getPack().splice(index, 1, itemHeld);
      return true;
    } else {
      this.equipped = itemToEquip;
